...
func _ready():
	...
	player.play("stop")
	ai.play("stop")
	role_object(labor, office_y, -200, 10, 60)
	role_object(soldier, cabin_y, 100, 100, 60)
	role_object(soldier, cabin_y, -300, -300, 60)
	role_object(soldier, cabin_y, -200, 150, 80)
	role_object(soldier, cabin_y, -400, -100, 100)
	pass
...
func _on_come_area_entered(area):
	if Global.level == 1:
		Global.level = 2
	get_tree().change_scene("res://scene/second_floor.tscn")
	pass
